...
from src.features.reader.models import GenerationMode, StorySession, StoryContext
...
class AdventureReaderFrame(ttk.Frame):
    ...
    def __init__(self, parent, controller, study_manager, db: FlashcardDatabase, llm_service=None):
        ...
        from src.features.reader.story_generator import StoryGenerator
        from src.features.reader.models import GenerationMode
        
        self.vocabulary_service = VocabularyService(db)
        # Default to TEMPLATE mode for initial generator
        self.story_generator = StoryGenerator(GenerationMode.TEMPLATE, db, llm_service)
        self.session_manager = StorySessionManager(db, self.vocabulary_service, self.story_generator)
    ...
    def show_session_setup(self):
        ...
        ttk.Radiobutton(mode_frame, text="AI Generated (High Quality)", variable=self.mode_var, value="llm").pack(anchor="w", pady=5)
    ...
    def _start_new_story(self):
        try:
            ...
            known_words = self.vocabulary_service.get_known_words(self.study_manager.study_language)
            if not known_words:
                messagebox.showwarning("Insufficient Vocabulary", "Please add words to your flashcard decks first.")
                return
            session_id = self.session_manager.create_session(language=self.study_manager.study_language, genre=genre, generation_mode=mode)
    ...
    def show_resume_menu(self):
        ...
        if not sessions:
            ...
            for session_id, genre, created_at, last_updated in sessions:
                ...
                session_text = f"📖 {genre.capitalize()} - {last_updated}"
                ...
                ttk.Button(session_button_frame, text="🗑️", command=lambda sid=session_id: self._delete_session(sid), width=3).pack(side="left", padx=5)
        back_frame = ttk.Frame(self)
        back_frame.pack(fill="x", padx=20, pady=20)
    ...
    def _delete_session(self, session_id: int):
        if messagebox.askyesno("Confirm Delete", "Delete this story?"):
            try:
    ...
    def show_reading_view(self):
        ...
        try:
            ...
            if history:
                ...
                known_words = self.vocabulary_service.get_known_words(self.current_session.language)
                constraints = VocabularyConstraints(known_words=known_words, min_coverage=0.90, max_new_words=2)
                
                # Ensure generator matches session mode
                self.story_generator.mode = self.current_session.generation_mode
                current_passage = self.story_generator.generate_passage(self.current_session.story_context, constraints)
            ...
            ttk.Label(scrollable_frame, text=f"Error: {str(e)}", font=("Arial", 11), foreground="red").pack(pady=20)
