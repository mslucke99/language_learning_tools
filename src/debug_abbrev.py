#!/usr/bin/env python
"""
Debug script for abbreviation handling
"""
import sys
import os
import regex
...
def debug_abbrev():
    ...
    for j, para in enumerate(paragraphs):
        ...
        for block_idx, block in enumerate(merged):
            ...
            print(f"    Checking for abbreviations/short fragments in: {merged_candidates}")
            
            for i, candidate in enumerate(merged_candidates):
                if skip_next:
                    print(f"      Skipping {repr(candidate)} (marked as skip_next)")
                    skip_next = False
                    continue
