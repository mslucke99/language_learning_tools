...
class _DeckListScreenState extends State<DeckListScreen> {
  ...
  Future<void> _loadDecks() async {
    ...
    // For each deck, count flashcards and due cards
    ...
    for (var deck in decks) {
      ...
      final dueCount = await _reviewService.getDueCardsCount(deck['id']);

      var deckMap = Map<String, dynamic>.from(deck);
      deckMap['cardCount'] = count;
      deckMap['dueCount'] = dueCount;
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final dueCount = deck['dueCount'] as int;
                return Card(
                  ...
                  child: Column(
                    children: [
                      ListTile(
                        ...
                        subtitle: Text(
                          '${deck['cardCount']} cards • ${deck['language'] ?? 'No language'}${dueCount > 0 ? ' • $dueCount due' : ''}',
                        ),
                                  ...
                                  Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                      builder: (context) => ExamSetupScreen(
                                        language: deck['language'] ?? 'Spanish',
                                      ),
                                    ),
                                  ).then((_) => _loadDecks());
                                },
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const DeckEditorScreen()),
          ).then((_) => _loadDecks());
        },
